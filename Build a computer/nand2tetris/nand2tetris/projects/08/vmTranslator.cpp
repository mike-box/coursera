...
std::string parseCommandBootstrap();
...
static void trace(int level,std::string text){
    if(level >= g_debug){
        cout<<text<<endl;
    }
    return;
...
static void debugEnable(int level){
    g_debug = level;
}
...
static bool checkMnemonics(std::string & s){
    for(char c : s){
...
static std::string binaryCode(int x,int len){
    std::string ans;
    for(int i = 0; i < len; ++i){
...
std::string parseCommandBootstrap(){
    std::string ans;
    string bootstrap = "call Sys.init 0";

    ans += "@256\n";
    ans += "D=A\n";
    ans += "@SP\n";
    ans += "M=D\n";
    ans += parseCommandCall(bootstrap) + "\n";
